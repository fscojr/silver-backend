...
@EqualsAndHashCode
@Entity
@Table(name = "tb_patio", schema = Constants.SCHEMA_PGADMIN_SILVER)
...
public class Patio implements Serializable {
    ...
    @Id
    @Column(name = "cd_seq_patio", nullable = false)
    @SequenceGenerator(name="tb_patio_cd_seq_patio_seq", sequenceName="tb_patio_cd_seq_patio_seq", 
    		schema = Constants.SCHEMA_PGADMIN_SILVER, allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "tb_patio_cd_seq_patio_seq")
    private Long id;
